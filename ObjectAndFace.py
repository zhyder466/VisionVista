    ...
    for encodeFace, faceLoc in zip(encodeCurFrame, faceCurFrame):
        ...
        if matches[matchIndex] and faceDis[matchIndex] < confidence_threshold:
            ...
            if name not in last_spoken_time or (current_time - last_spoken_time[name] > cooldown_period):
                ...
                speech_thread = threading.Thread(target=generate_speech, args=(f"I can see {name}.",))
                speech_thread.start()
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            if confidence >= confidence_threshold:
                ...
                cv2.putText(img, f"{class_name} ({confidence:.2f})", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                ...
                if class_name == "person" and not known_person_detected:
                    ...
                    if class_name not in last_spoken_time or (current_time - last_spoken_time[class_name] > cooldown_period):
                        ...
                        speech_thread = threading.Thread(target=generate_speech, args=(f"I can see a {class_name}.",))
                        speech_thread.start()
